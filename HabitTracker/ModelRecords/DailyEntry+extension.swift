//
//  DailyHabitEntry.swift
//  HabitTracker
//
//  Created by Volodymyr Boichentsov on 29/09/2024.
//

import Foundation
import SwiftData


extension DailyEntry {
    var title:String {
        return self.habitt.title
    }
}

extension SchemaV3.HabitItem {
    var targetCount:Int {
        get { return 1 }
        set {}
    }
}

extension SchemaV3.DailyEntry {
    
    var achievement:Achievement? {
        get { return Achievement.none }
        set {}
    }
    
    func setCompleted(_ value:Bool) {
        self.isCompleted = value
        if value {
            self.completionDate = Date()
        } else {
            self.completionDate = nil
        }
    }
    
    var completionDates:[Date] {
        get {
            return (completionDate != nil) ? [completionDate!] : []
        }
        set {
            self.completionDate = newValue.first
        }
    }
}

extension SchemaV4.DailyEntry {
    func setCompleted(_ value: Bool) {
    
    if value && !isCompleted {
        let now = Date()
        // Preserve time components from now but use date components from self.date
        let calendar = Calendar.current
        var components = calendar.dateComponents([.hour, .minute, .second], from: now)
        let dateComponents = calendar.dateComponents([.year, .month, .day], from: self.date)
        
        components.year = dateComponents.year
        components.month = dateComponents.month
        components.day = dateComponents.day
        
        if let newDate = calendar.date(from: components) {
            completionDates.append(newDate)
        }
    } else if !value {
        completionDates.removeAll()
        achievement = Achievement.none
    }
}
}

func sortDailyHabits(item1: DailyEntry, item2: DailyEntry) -> Bool {
    if item1.habitt.isTimeSensitive != item2.habitt.isTimeSensitive {
        return item1.habitt.isTimeSensitive // Time-sensitive items come first
    }
    
    if let time1 = item1.habitt.time, let time2 = item2.habitt.time {
        // Both times are non-nil; compare directly
        if time1 != time2 {
            return time1 < time2
        }
    } else if item1.habitt.time != nil {
        // item1 has a time, item2 does not; item1 should come first
        return true
    } else if item2.habitt.time != nil {
        // item2 has a time, item1 does not; item2 should come first
        return false
    }
    
    // If times are equal or both are nil, fall back to order
    return item1.habitt.order < item2.habitt.order
}
